...
BEGIN
    ...
    WHERE u.id = (
        SELECT CASE 
            WHEN completed_by_user_id = (SELECT MAX(id) FROM users) 
            THEN (SELECT MIN(id) FROM users)
            ELSE (SELECT MIN(id) FROM users WHERE id > completed_by_user_id)
        END
    );
...
BEGIN
    ...
    IF approved THEN
        ...
        -- Activate the next assignment we created
        UPDATE task_assignments 
        SET is_active = true, assigned_at = CURRENT_TIMESTAMP
        WHERE task_id = task_id_var 
          AND is_active = false
          AND id = (
              SELECT MAX(id) FROM task_assignments 
              WHERE task_id = task_id_var AND is_active = false
          );
